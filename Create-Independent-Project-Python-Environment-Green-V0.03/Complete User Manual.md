...
**Portable / green edition V0.03:** double-click `Create Independent Project Python Environment.exe` in this folder. No `.lnk` shortcut and no extra `.bat` for the end user.

Chinese name: **双击生成独立项目的 Python 环境（绿色版 V0.03）**
...
4. Leave a per-project switch for how `.py` files are double-clicked
...
## V0.03: maintain this project’s libraries

After a project is created, double-click **维护本项目的库.bat** in that project folder.

- Tick common libraries, or paste names from PyPI into the box (spaces or commas).
...
- Do not double-click `维护本项目的库.py` if you only want the window; that file is the program. Use the `.bat`.

This window never installs into the computer’s system Python.

Older projects made with V0.02 do not get this window automatically. Create a new project with V0.03, or copy the two files from a new project if you want the same window.
...
Windows cannot change double-click for only one folder. The first time you create a project with this tool, a small opener is stored under your user AppData. It only looks at **that project’s** enable file (`use_project_env.on`). If the project is not enabled, double-click still behaves like the system default.
...
生成成功后，在新项目文件夹里双击 **维护本项目的库.bat**：

- 上面勾常用库，或把库名粘贴进输入框
- 下面只列出你装过的库；它们需要的小库不单独列
- 没装完的不会写成「已安装」
- 请双击 `.bat`，不要双击旁边那个 `.py`（那个是程序本身）

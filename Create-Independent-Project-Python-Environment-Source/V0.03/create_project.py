"""Green edition V0.03: name a folder, create that project's Python environment, library window, optional per-project .py switch."""
...
LIB_PY_NAME = "维护本项目的库.py"
LIB_BAT_NAME = "维护本项目的库.bat"
LIB_BUNDLE_NAME = "manage_project_libs.py"
...
def library_source() -> Path:
    here = tool_dir()
    for name in (LIB_PY_NAME, LIB_BUNDLE_NAME):
        path = here / name
        if path.is_file():
            return path
    raise ValueError("Missing 维护本项目的库.py; cannot add the library window.")


def write_library_manager(dest: Path) -> None:
    shutil.copy2(library_source(), dest / LIB_PY_NAME)
    bat_body = (
        "@echo off\r\n"
        "cd /d \"%~dp0\"\r\n"
        "if exist \"%~dp0.venv\\Scripts\\pythonw.exe\" (\r\n"
        "  start \"\" \"%~dp0.venv\\Scripts\\pythonw.exe\" \"%~dp0%~n0.py\"\r\n"
        "  exit /b 0\r\n"
        ")\r\n"
        "if exist \"%~dp0.venv\\Scripts\\python.exe\" (\r\n"
        "  start \"\" \"%~dp0.venv\\Scripts\\python.exe\" \"%~dp0%~n0.py\"\r\n"
        "  exit /b 0\r\n"
        ")\r\n"
        "echo Could not find this project's .venv\r\n"
        "pause\r\n"
    )
    (dest / LIB_BAT_NAME).write_text(bat_body, encoding="ascii")
...
def open_window() -> None:
    ...
    def on_create() -> None:
        ...
        messagebox.showinfo(
            ...
            "To add or remove libraries, double-click:\n"
            f"{LIB_BAT_NAME}\n\n"
            + extra,
        )
